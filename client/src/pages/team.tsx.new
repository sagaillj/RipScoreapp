...
const TeamSwitcher = ({ teams, currentTeamId, onTeamChange, onAddTeam }: { 
  ...
  // This ref is used to update the open state from outside this component
  useEffect(() => {
    // Expose a global function to open the dropdown
    (window as any).openTeamDropdown = () => setOpen(true);
  }, []);
...
export default function Team() {
  ...
  useEffect(() => {
    if (teamsData && Array.isArray(teamsData) && teamsData.length > 0) {
      ...
      if (team) {
        ...
        // Preserve the default app accent colors - don't change them
        // Only set custom colors if specifically requested
        
        // Set colors but preserve the defaults
        setPrimaryColor(DEFAULT_PRIMARY_COLOR);
        setSecondaryColor(DEFAULT_SECONDARY_COLOR);
        
        // Reset any custom document CSS variables to defaults
  ...
  const handlePermanentDelete = () => {
    if (diverToDelete) {
      // Update UI immediately for better user experience
      setDivers(prevDivers => 
        prevDivers.filter(d => d.id !== diverToDelete.id)
      );
      
      // Also remove the diver's image if it exists
      if (diverImages[diverToDelete.id]) {
        setDiverImages(prev => {
          const updated = {...prev};
          delete updated[diverToDelete.id];
          return updated;
        });
      }
      
      // Delete from database
      deleteDiverMutation.mutate(diverToDelete.id);
      
      toast({
        title: "Diver Deleted",
        description: `${diverToDelete.firstName} ${diverToDelete.lastName} has been permanently deleted`
      });
  ...
  const handleSaveTeamSettings = () => {
    ...
    // Close the dialog first to prevent state conflicts
    ...
    // Create team data for API update
    const teamData = {
      name: teamName,
    ...
    // Log the updated team for debugging
    console.log("Updating team:", teamData);
    
    // Use the mutation to update the team in the database
    updateTeamMutation.mutate(teamData);
    
    // Force a re-render with DOM manipulation for immediate UI update
    // This will be handled by React Query refresh after API response
    setTimeout(() => {
      const nameElements = document.querySelectorAll('h1');
      nameElements.forEach(el => {
        if (el.textContent?.includes('Gold Dolphins') || 
            el.textContent?.includes('Blue Waves') || 
            el.textContent?.includes('Eagle Divers')) {
          el.innerHTML = teamName + el.innerHTML.substring(el.textContent.length);
        }
      });
      
      // Update the page title too
      document.title = teamName + ' - RipScore';
      
      // Force another component re-render
      setForceUpdate({});
    }, 50);
  };
    ...
    <DashboardLayout>
              ...
              <h1 className="text-2xl font-bold text-white flex items-center">
                ...
                {teams.length > 1 && (
                  <button 
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      if (typeof (window as any).openTeamDropdown === 'function') {
                        (window as any).openTeamDropdown();
                      }
                    }}
                  >
      ...
      <Dialog open={teamSettingsOpen} onOpenChange={setTeamSettingsOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
          ...
          <DialogFooter className="mt-4 flex flex-col space-y-4">
      ...
      <Dialog open={deleteTeamDialogOpen} onOpenChange={setDeleteTeamDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          {diverToDelete && (
            <div className="py-4">
              <p className="mb-4">
                Are you sure you want to permanently delete <span className="font-semibold">{diverToDelete.firstName} {diverToDelete.lastName}</span>?
              </p>
              
              <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-md border border-red-200 dark:border-red-800">
                <p className="text-sm text-red-600 dark:text-red-400">
                  <strong>Warning:</strong> This will delete all dive records, scores, and history associated with this diver. 
                  Consider archiving instead to maintain historical data.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="mt-6 flex justify-between items-center">
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  // Include status and other fields
